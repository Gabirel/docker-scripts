...
set private_registry_url 'localhost:5000'

function export_images
    set counter 1
    for image in $images
        set time (date +"%m-%d %H:%M:%S")
        echo "[*] $time Start to export <$counter> image: $image"
        docker save -o /docker_dataset/$image.tar $image
        set time (date +"%m-%d %H:%M:%S")
        echo -e "[*] $time Finished exporting <$counter> image: $image\n"
